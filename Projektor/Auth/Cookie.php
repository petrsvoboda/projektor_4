...
class Projektor_Auth_Cookie {
        ...
        if($userid) {
            ...
            if(array_key_exists(self::$cookiename, $_COOKIE)) {
               $this->_unpack($_COOKIE[self::$cookiename]);
                ...
                throw new Projektor_Auth_Exception ("Není cookie");
            }
        }
    }
    ...
    public function validate() {
        if(!$this->version || !$this->created || !$this->userid) {
            throw new Projektor_Auth_Exception("Poškozená cookie");
        }
        ...
        if (time() - $this->created > self::$expiration) {
            throw new Projektor_Auth_Exception("Vypršl čas platnosti cookie");
        }
    ...
    private function _unpack($cookie) {
        ...
        if($this->version != self::$myversion || !$this->created || !$this->userid) {
            throw new Projektor_Auth_Exception();
        }
    }
