<?php
class Projektor_Controller_Stranka_ISCO_Seznam extends Projektor_Controller_Stranka_Seznam
{
	...
	protected function potomekNeni()
	{
		...
		$tlacitka = array
		(
                        new Projektor_Controller_Stranka_Element_Tlacitko("Vyhledání ISCO", $this->uzel->potomekUri("Projektor_Controller_Stranka_ISCOM_Seznam_Vyhledani"))
		);
                $this->novaPromenna("tlacitka", $tlacitka);
                $this->generujSeznam();
	}

        public function potomek°Projektor_Controller_Stranka_ISCO_Seznam(Projektor_Controller_Uzel $uzelPotomek = null)
        {
//                        $kod = str_replace(" ", "", substr($jednoisco->kod, 0, 4));
//            $jednoisco->odkaz = $this->uzel->potomekUri("Projektor_Controller_Stranka_ISCOM_Seznam", array("id" => $jednoisco->id, "prefixKodu" => $kod));
                /* Nadpis stranky */
        ...
        public function potomek°Projektor_Controller_Stranka_ISCO_Detail(Projektor_Controller_Uzel $uzelPotomek = null)
	{
                /* Nadpis stranky */
        ...
        protected function potomek°Projektor_Controller_Stranka_ISCO_Seznam_Vyhledani()
        {
        ...
        {
            $kod = str_replace(" ", "", substr($jednoisco->kod, 0, 4));
            $jednoisco->odkaz = $this->uzel->potomekUri("Projektor_Controller_Stranka_ISCO_Seznam", array("id" => $jednoisco->id, "prefixKodu" => $kod));
            $jednoisco->tlacitka = array
                (
                    new Projektor_Controller_Stranka_Element_Tlacitko("Úroveň ".(strlen($kod)+1), $jednoisco->odkaz),
                );
        }
	...
	protected function generujTlacitkaProPolozku($jednoisco)
        {
            $jednoisco->tlacitka = array
                (
                        new Projektor_Controller_Stranka_Element_Tlacitko("Detail", $this->uzel->potomekUri("Projektor_Controller_Stranka_ISCO_Detail", array("id" => $jednoisco->id, "zmraz" => 1))),
                );
        }
        ...
        {
                /* Hlavicka tabulky */
		$hlavickaTabulky = new Projektor_Controller_Stranka_Element_Hlavicka($tridaData, $this->uzel);
