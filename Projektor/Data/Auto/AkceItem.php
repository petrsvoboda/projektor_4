<?php
class Projektor_Data_Auto_AkceItem extends Projektor_Data_Item
{
...
//	 * @return Projektor_Data_Akce Instance tridy obsahujici data z radku v tabulce
//	 */
//
...
//                    return false;
//
//		return new Projektor_Data_Akce($radek[self::NAZEV_HLAVNIHO_OBJEKTU], $radek[self::DATUM_ZACATEK], $radek[self::DATUM_KONEC], $radek[self::NAZEV], $radek[self::POPIS], $radek[self::ID_S_STAV_AKCE_FK], $radek[self::ID_S_TYP_AKCE_FK], $radek[self::ID]);
//	}
//
//
//	/**
...
//	}
	 ...
	 * @return Projektor_Data_Seznam_STypAkce
	 */

	public function dejSTypAkce()
	{
		return Projektor_Data_Seznam_STypAkce::najdiPodleId($this->idSTypAkceFK);
	}

	/**
	 * Vrati instanci stavu teto akce.
	 * @return Projektor_Data_Seznam_SStavAkce
	 */

	public function dejSStavAkce()
	{
		return Projektor_Data_Seznam_SStavAkce::najdiPodleId($this->idSStavAkceFK);
	}
	...
	public function prihlas($ucastnik, $sStavUcastnikAkce, $sStavUcastnikAkceDen)
	{
		...
		$dbh->prepare($query)->execute(Projektor_Data_Auto_VzbUcastnikAkceCollection::TABULKA, Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_UCASTNIK_FK,
		Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_AKCE_FK, Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_S_STAV_UCASTNIK_AKCE_FK,
	 ...
	 * @param Projektor_Data_Seznam_SStavUcastnikAkce $sStavUcastnikAkce Stav ucastnika vzhledem k Akci
	 * @param Projektor_Data_Seznam_SStavUcastnikAkceDen $sStavUcastnikAkceDen Stav ucastnika vzhledm k dnum Akce (AkceDen) - volitelne
	 * @return unknown_type
	 */

	public function zmenStavUcastnika($ucastnik, $sStavUcastnikAkce, $sStavUcastnikAkceDen = null)
	{
		if(!Projektor_Data_Seznam_SPrechodUcastnikAkce::jeMozny($this->stavUcastnika($ucastnik), $sStavUcastnikAkce))
		...
		$dbh->prepare($query)->execute(Projektor_Data_Auto_VzbUcastnikAkceCollection::TABULKA,
		Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_S_STAV_UCASTNIK_AKCE_FK,
		$sStavUcastnikAkce->id,
		Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_UCASTNIK_FK,
		$ucastnik->id,
		Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_AKCE_FK,
		$this->id);
	...
	public function stavUcastnika($ucastnik)
	{
		...
		$radek = $dbh->prepare($query)->execute(Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_S_STAV_UCASTNIK_AKCE_FK, Projektor_Data_Auto_VzbUcastnikAkceCollection::TABULKA,
		Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_UCASTNIK_FK, $ucastnik->id,
		Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_AKCE_FK, $this->id
		)->fetch_assoc();

		if($radek)
			return Projektor_Data_Seznam_SStavUcastnikAkce::najdiPodleId($radek[Projektor_Data_Auto_VzbUcastnikAkceCollection::ID_S_STAV_UCASTNIK_AKCE_FK]);
		else
			return false;
	}
	 ...
	 * @param Ucastnik $ucastnik Instance ucastnika, kteremu menime stav
	 * @param Projektor_Data_Seznam_SStavUcastnikAkceDen $sStavUcastnikAkceDen Stav ucastnika vzhledem k dnum Akce (AkceDen)
	 * @return unknown_type
	 */
	 ...
	 * @param Projektor_Data_Seznam_SStavUcastnikAkce $sStavUcastnikAkce Stav ucastniku vzhledem k Akci
	 * @param Projektor_Data_Seznam_SStavUcastnikAkceDen $sStavUcastnikAkceDen Stav ucastniku vzhledm k dnum Akce (AkceDen) - volitelne
	 * @return unknown_type
	 */

	public function zmenStav($sStavAkce, $sStavUcastnikAkce = null, $sStavUcastnikAkceDen = null)
	{
		if(!Projektor_Data_Seznam_SPrechodAkce::jeMozny($this->dejSStavAkce(), $sStavAkce))
	...
	public function vytvorDny($sUcebna, $sPersonal)
	{
		...
		{
			$akceDen = new Projektor_Data_AkceDen($datum->format("Y-m-d"), $this->id, $sUcebna->id, $sPersonal->id);
			$akceDen->uloz();
			$datum->modify("+1 day");
		}
	...
	{
		return Projektor_Data_AkceDen::vypisVse(Projektor_Data_AkceDen::ID_AKCE_FK."={$this->id}");
	}
}
?>
