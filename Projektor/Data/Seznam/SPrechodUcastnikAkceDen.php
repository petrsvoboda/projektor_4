...
class Projektor_Data_Seznam_SPrechodUcastnikAkceDen extends Projektor_Data_Item
{
	public $id;
	public $text;
	public $plnyText;
	...
	const TABULKA = "s_prechod_ucastnik_akce_den";
	const ID = "id_s_prechod_ucastnik_akce_den";
	const TEXT = "text";
	...
	public static function najdiPodleId($id)
	{
		...
		return new Projektor_Data_Seznam_SPrechodUcastnikAkceDen($radek[self::TEXT], $radek[self::PLNY_TEXT],
	...
	public static function smaz($SPrechodUcastnikAkceDen)
	{
		$dbh = Projektor_App_Kontext::getDbh(Projektor_App_Config::DATABAZE_PROJEKTOR);
		$query = "UPDATE ~1 SET valid = 0 WHERE ~2=:3";
		$dbh->prepare($query)->execute(self::TABULKA, self::ID, $SPrechodUcastnikAkceDen->id);
	}


	/**
	 * Overi jestli je mozne prejit z jednoho stavu do druheho.
	 * @param Projektor_Data_Seznam_SStavUcastnikAkceDen $stavPred Stav ucastnika pred prechodem, tj.soucasny stav.
	 * @param Projektor_Data_Seznam_SStavUcastnikAkceDen $stavPo Stav ucastnika po prechodu, tj budouci stav.
