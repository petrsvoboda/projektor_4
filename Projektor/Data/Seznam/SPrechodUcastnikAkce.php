...
class Projektor_Data_Seznam_SPrechodUcastnikAkce extends Projektor_Data_Item
{
	public $id;
	public $text;
	public $plnyText;
	...
	const TABULKA = "s_prechod_ucastnik_akce";
	const ID = "id_s_prechod_ucastnik_akce";
	const TEXT = "text";
	...
	public static function najdiPodleId($id)
	{
		...
		return new Projektor_Data_Seznam_SPrechodUcastnikAkce($radek[self::TEXT], $radek[self::PLNY_TEXT],
	...
	public static function smaz($SPrechodUcastnikAkce)
	{
		$dbh = Projektor_App_Kontext::getDbh(Projektor_App_Config::DATABAZE_PROJEKTOR);
		$query = "UPDATE ~1 SET valid = 0 WHERE ~2=:3";
		$dbh->prepare($query)->execute(self::TABULKA, self::ID, $SPrechodUcastnikAkce->id);
	}

	/**
	 * Overi jestli je mozne prejit z jednoho stavu do druheho.
	 * @param Projektor_Data_Seznam_SStavUcastnikAkce $stavPred Stav ucastnika pred prechodem, tj.soucasny stav.
	 * @param Projektor_Data_Seznam_SStavUcastnikAkce $stavPo Stav ucastnika po prechodu, tj budouci stav.
