...
echo interval();
    if (isset($_GET['cesta']))
    {
        echo Projektor_Request_Cesta::getContent($_GET['cesta']);
    } else {
//        $koren = new Projektor_Router_Uzel("Projektor_Controller_Strom_AkceJ_Menu_Detail", null, array("id" => 5), FALSE);
        $koren = new Projektor_Controller_Strom_Uzel("Projektor_Controller_Strom_PrezentaceJ_Detail", null, array("id" => 5), FALSE);
        $cesta_serialized = serialize($koren);
        echo Projektor_Request_Cesta::getContent($cesta_serialized);
    }
